...
class gamelist{
	...
	*/
	public static function add_game($game_id){
		...
		$res = self::$game_db->query($sql);
		game_stat::check_error($sql);
		self::update_user(array("status"=>$_SESSION["status"],"game_id"=>self::$game_db->lastInsertRowID()));
	}
	...
	public static function add_user(){
		...
		if($usr["count"]){
			$sql = "UPDATE players SET status =  ".$_SESSION["status"].", game_id =  ";
			...
			$sql.= $_SESSION["status"].", \"".($_SESSION["gameId"]?$_SESSION["gameId"]:"null")."\")" ;
		}
		$res = self::$game_db->query($sql);
		game_stat::check_error($sql);
	}
